...
void pumpStop();
void drainFluid(void *parameter);
...
void dataCallback(StaticJsonDocument<200> doc) {
  ...
  if (doc["msg"] == "pump1" || doc["msg"] == "pump2") {
    ...
    xTaskCreatePinnedToCore(getPumpReady, "handlePumpData", 4096,
                            (void *)handlepump, 1, &pump_Task,
                            0);  // ส่ง pointer ไปยัง task
  } else if (doc["msg"] == "Drain1" || doc["msg"] == "Drain2") {
    dataPump *handlepump = new dataPump;
    handlepump->msg = doc["msg"].as<String>();

    if (handlepump->msg == "Drain1") {
      handlepump->cell = 1;
      handlepump->door = Door_1;
      handlepump->pump_pin = Pump_1;
      handlepump->valve_pin = Valve_1;
    } else if (handlepump->msg == "Drain2") {
      handlepump->cell = 2;
      handlepump->door = Door_2;
      handlepump->pump_pin = Pump_2;
      handlepump->valve_pin = Valve_2;
    }
    xTaskCreatePinnedToCore(drainFluid, "Drain Fluid", 1024, (void *)handlepump,
                            1, &drainFluid_task, 0);
    ...
    xTaskCreatePinnedToCore(saveSwapData, "saveSwapData", 4096,
                            (void *)handleSwap, 1, &swap_Task, 0);
  } else if (doc["msg"] == "Cal") {
    ...
    xTaskCreatePinnedToCore(Calibration, "Calibration", 4096, (void *)handleCal,
                            1, &cal_Task, 0);
  }
}
...
void setup() {
  ...
  xTaskCreatePinnedToCore(mqttLoop, "MQTT_LOOP", 4096, NULL, 1, &MQTT_loop_task,
                          0);
  xTaskCreatePinnedToCore(NTPloop, "NTPloop", 2048, NULL, 1, &NTP_loop, 0);
  // xTaskCreatePinnedToCore(IdelWeightCheck, "IdelWeightCheck", 2014, NULL, 1,
  // &IdelWeightCheck_task, 0);
  end = micros();
  delta = end - start;
...
void IdelWeightCheck(void *parameter) {
  while (1) {
    ...
    float weight1 =
        hx711Reader::getInstance()->readData(1, 0, calibrationFactor1);
    float weight2 =
        hx711Reader::getInstance()->readData(2, 0, calibrationFactor2);
...
void drainFluid(void *parameter) {
  ...
  while (1) {
    startSwitch = digitalRead(Ready_pump);

    if (startSwitch == false) {
      pumpStart(&handlePump, 10000);
    } else {
      pumpStop();
    }
    vTaskDelay(xDelay100ms);
...
void getPumpReady(void *parameter) {
  ...
  xTaskCreatePinnedToCore(CheckPumpStatus, "CheckPumpStatus", 4096,
                          reinterpret_cast<void *>(&handlePump), 1, &cal_Task,
                          0);
  ...
  //   hx711Reader::getInstance()->readData(handlePump.cell, 0,
  //   calibration_factor);
  // #endif
...
void CheckPumpStatus(void *parameter) {
  ...
  float previousWeight = hx711Reader::getInstance()->readData(
      handlePump.cell, 0, calibrationFactor);  // เก็บค่าน้ำหนักก่อนหน้า
  ...
  while (1) {
    ...
    if (!readyPumpStatus && !doorClosed && error == false) {
      ...
      float currentWeight = hx711Reader::getInstance()->readData(
          handlePump.cell, 0, calibrationFactor);
      ...
      // ถ้าไม่มีการไหลของของเหลวเกิน 10 g เกิน 1 นาที ให้สถานะ error true
      if (currentWeight - previousWeight < 10 &&
          currentTime - previousTime > 60000 && error == false) {
        pumpStop();
        Serial2.println(200);
      ...
      // ถ้ามีการไหลของของเหลวเกิน 10 g เกิน 1 นาที
      // อ่านค่าน้ำหนักและค่าเวลาเก็บไว้ในตัวแปลเก็บค่าก่อนหน้า
      else if (currentWeight - previousWeight > 10 &&
               currentTime - previousTime > 60000) {
        previousWeight = hx711Reader::getInstance()->readData(
            handlePump.cell, 0, calibrationFactor);
        previousTime = millis();
      }
    ...
    if (doorClosed) {
      ...
      if (currentWeight <= 5 && error == true) {
        ...
        dataSender::getInstance()->sendPumpMessage(
            handlePump.cell, handlePump.value, error, liquidLevel);
        vTaskDelete(NULL);
      } else {
        sendStartMQTT = false;
...
void Calibration(void *parameter) {
  ...
  if (handleCal.cal_msg.toInt() == 9 || handleCal.cal_msg.toInt() == 10) {
    hx711Reader::getInstance()->FindZeroFactor((handleCal.cal_msg.toInt() % 9) +
                                               1);
  } else {
#ifdef LOAD_CELL_ACTIVE
    handelCalConstant = hx711Reader::getInstance()->FindCalibrationFactor(
        handleCal.value, cell);
  ...
  dataSender::getInstance()->sendCalData(cell, handleCal.cal_msg.toInt(),
                                         handleCal.value, 0);
  end = micros();
  delta = end - start;
