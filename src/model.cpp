...
WiFiClient *myWiFiManager::getClient() { return &wifiClient; }
...
void MQTTManager::setServer(const char *server, int port, const char *username,
                            const char *password) {
...
void MQTTManager::connect() {
  ...
  while (!client->connected()) {
    ...
    if (client->connect("ESP32Client", mqttUsername, mqttPassword, NULL, 2,
                        NULL, NULL)) {
    ...
    if (millis() - startAttemptTime > mqttTimeout) {  // 30 seconds timeout
...
void MQTTManager::setMessageCallback(void (*callback)(char *, byte *,
                                                      unsigned int)) {
